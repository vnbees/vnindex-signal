...
- Quy tắc xuất kết quả:
  ...
  * Khi xuất JSON cho pipeline automation: bắt buộc có trường top-level `reason` (tiếng Việt, tối thiểu 300 ký tự) — **phải nhắc tên mã (HOÁ) từng mã trong `selected_signals`** và số liệu cụ thể (RSI, volume_ratio, sector-flow…); cấm chỉ lặp lại mô tả quy trình lọc chung. Giải thích `near_miss_signals` và thứ tự ưu tiên; nếu danh sách mã đạt rỗng thì `reason` phải nêu rõ vì sao (lưu ý ingest hiện yêu cầu ít nhất một mã đạt).
