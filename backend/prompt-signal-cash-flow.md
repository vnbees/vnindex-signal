...
- Quy tắc xuất kết quả:
  ...
  * Khi xuất JSON cho pipeline automation: bắt buộc có trường top-level `reason` (tiếng Việt, tối thiểu 200 ký tự) — giải thích chi tiết toàn bộ suy luận dẫn tới `selected_signals`, `near_miss_signals`, và thứ tự ưu tiên; nếu danh sách mã đạt rỗng thì `reason` phải nêu rõ vì sao.
