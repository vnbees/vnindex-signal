...
def calc_seasonal_score(today: date) -> Tuple[int, Dict]:
    ...
    return score, {"month": today.month, "dayOfWeek": dow + 1, "reason": reason}
...
def display_results(signals: List[Dict]):
    ...
    print(f"\n{'─'*80}")
    print(f"📊 Phân tích {len(signals)} mã HOSE thanh khoản tốt | {TODAY.strftime('%Y-%m-%d')}")
    print(f"Backtest: 80,734 obs mean-reversion (2023-2026) | Hold: 3 ngày giao dịch")
...
async def main():
    ...
    day_names = {0: "Thứ Hai", 1: "Thứ Ba", 2: "Thứ Tư", 3: "Thứ Năm", 4: "Thứ Sáu", 5: "Thứ Bảy", 6: "Chủ Nhật"}
    print(f"   Ngày: {TODAY.strftime('%Y-%m-%d')} ({day_names.get(TODAY.weekday(), '')}) | Hold: {HOLD_DAYS}d | Backend: {WEBSITE_URL}")
    print("=" * 80)
