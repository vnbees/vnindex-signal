...
export default async function VonItDetailPage({ params }: Props) {
  const { date, symbol } = params;

  let signal;
  try {
    signal = await getSignalDetail(date, symbol.toUpperCase(), PORTFOLIO_LOW);
  } catch {
    notFound();
  }
  ...
  return (
    <div className="max-w-4xl">
      <div className="text-xs text-tv-muted mb-4 tracking-wide">
        <Link href="/danh-muc-von-it" className="hover:text-tv-accent">
          Danh mục vốn ít
        </Link>
        <span className="mx-1.5 text-tv-border">/</span>
        <Link href={`/danh-muc-von-it/${date}`} className="hover:text-tv-accent">
          {formatDate(date)}
        </Link>
        <span className="mx-1.5 text-tv-border">/</span>
        ...
        <div className="tv-panel p-5">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-sm font-medium text-tv-text">
                {signal.price_open_t1 ? (
                  formatPrice(signal.price_open_t1)
                ) : (
                  <span className="text-tv-muted">Chưa có</span>
                )}
              </span>
            </div>
